...
function optimize(keep: CardEx[], consider: CardEx[], budget: number, log: boolean = false): HandValue {
    ...
    // i.e., for any two consequtive cards it holds true
    // Deck.order(keep[i].card, keep[i+1].card) > 0

    // exclude optional log branch for coverage
    /* istanbul ignore next */
    if(log){
    ...
    for(let i = 0; i < consider.length; i++) {
        ...
        // Due ot Guarantee 1:
        // face value of the next card is either the same or increased,
        ...
        // Guarantee 2: we are not keeping in hand more than budget
...
export { best_hand, optimize, Loc, CardEx };
